...
export default function Enquiry() {
  ...
  const [all_enquiry_details, setall_enquiry_details] = useState([]);

  const get_all_enquiry_details = async () => {
    try {
      setLoading(true);
      const resp = await api.get(
        `api/contactus/Get-enquiry?page=${page}&limit=${limit}&search=${searchTerm}`
      );

      setall_enquiry_details(resp.data.enquiry);
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
          ...
          <div className="bg-white rounded-lg shadow p-4">
            <div className="text-2xl font-bold text-purple-600">
              {Array.isArray(all_enquiry_details)
                ? all_enquiry_details.filter(
